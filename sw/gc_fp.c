...
best_t score_frameplus_p2n_opt(seqtype* nseq, ms_profile_t *prof, int profLen, int seqLen)
{
	/* What I know about the sequence:
	1. before we do anything, nucleic seq. is prepened by a blank symbol.
	2. Next, each position [i] is subst. with AA corresponding to [i-2,i-1, i]. It follows that the first 3 positions are always 'X' (no corr. AA).
...
double fp_gencore_seq(const search_fp_profile_t * sp, const sequence_t * dseq, const sequence_t * qseq)
{
	...
	inpf_t max_v;
	best_t max_s = score_frameplus_p2n_opt(ms_sequence, ms_profile, prof_length, dseq->len + 1);
	max_v.score = ((float)(max_s.score)) * ms_profile->scaleback;
	max_v.x = max_s.x;
	max_v.y = max_s.y;
	max_v.state = -1; /* unknown */
